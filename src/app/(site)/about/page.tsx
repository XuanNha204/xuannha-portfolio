...
function TimelineItem({
  ...
  return (
    <RevealItem className="relative pl-8">
      ...
      <span className="absolute left-[5px] top-6 h-[calc(100%-0.75rem)] w-0.5 bg-border" />
      <p className="font-mono text-xs text-muted">{period}</p>
      <h3 className="mt-1 font-heading text-lg font-semibold text-primary">{title}</h3>
    ...
    <>
            ...
            <Reveal direction="left">
              ...
              {profile.resumeUrl && (
                <a
                  ...
                  className="mt-6 inline-flex h-11 items-center gap-2 rounded-lg bg-primary px-6 text-sm font-medium text-white shadow-md transition-all hover:-translate-y-0.5 hover:bg-secondary"
                >
                  <Download className="h-4 w-4" /> Tải CV của tôi
                </a>
              )}
            </Reveal>
          </div>
      ...
      {profile.careerGoal && (
        <section className="py-16">
          <div className="container-page">
            <Reveal>
              <div className="flex gap-4 rounded-xl border border-border bg-surface p-6 shadow-[var(--shadow-card)] md:p-8">
        ...
        <div className="container-page grid gap-16 lg:grid-cols-2">
          <div>
            <Reveal className="mb-8 flex items-center gap-3">
              ...
              <h2 className="font-heading text-2xl font-bold text-primary">Kinh nghiệm</h2>
            </Reveal>
          ...
          <div>
            <Reveal className="mb-8 flex items-center gap-3">
              ...
              <h2 className="font-heading text-2xl font-bold text-primary">Học vấn</h2>
            </Reveal>
            {educations.length === 0 ? (
            ...
            <RevealGroup className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {certificates.map((cert) => (
                <RevealItem key={cert._id}>
                  <div className="group flex h-full flex-col rounded-xl border border-border bg-surface p-6 shadow-[var(--shadow-card)] transition-all hover:-translate-y-1 hover:shadow-[var(--shadow-card-hover)]">
                    <span className="mb-4 flex h-10 w-10 items-center justify-center rounded-lg bg-warning/10 text-warning">
                      <Award className="h-5 w-5" />
                    </span>
                    <h3 className="font-heading font-semibold text-primary">{cert.name}</h3>
